...
func TestTemplateGenerationWithUpdatedVersions(t *testing.T) {
  ...
  "build": {
	...
	if dependencies["next"] != "15.5.2" {
		t.Errorf("Next.js version not correctly substituted: got %v, expected 15.5.2", dependencies["next"])
	}
...
func TestTemplateUpdatePerformance(t *testing.T) {
  ...
  "dependencies": {
	...
	for i := 0; i < templateCount; i++ {
		...
		if !validationResult.Valid {
			t.Errorf("Template %d validation failed", i)
		}
	}
...
func TestMultipleTemplateConsistency(t *testing.T) {
  ...
  "devCommand": "npm run dev"
	...
	if !consistencyResult.Valid {
		t.Errorf("Template consistency validation failed:")
	...
	for templateName, projectDir := range generatedProjects {
		...
		if !projectResult.Valid {
			t.Errorf("Project %s validation failed:", templateName)
