...
// FilesystemStorage implements cache storage using the filesystem.
...
func NewFilesystemStorage(cacheDir string, config *interfaces.CacheConfig) *FilesystemStorage {
...
// Initialize sets up the filesystem storage.
func (fs *FilesystemStorage) Initialize() error {
	...
	testFile := filepath.Join(fs.cacheDir, ".write_test")
...
// Load loads cache data from filesystem.
func (fs *FilesystemStorage) Load() (map[string]*interfaces.CacheEntry, *interfaces.CacheMetrics, error) {
	cacheFilePath := filepath.Join(fs.cacheDir, "cache.json")
...
// Save saves cache data to filesystem.
func (fs *FilesystemStorage) Save(entries map[string]*interfaces.CacheEntry, metrics *interfaces.CacheMetrics) error {
	if !fs.config.PersistToDisk {
		return nil
	}
...
// Clear removes all cache data from filesystem.
...
func (fs *FilesystemStorage) Backup(path string, entries map[string]*interfaces.CacheEntry, metrics *interfaces.CacheMetrics) error {
...
func (fs *FilesystemStorage) Restore(path string) (map[string]*interfaces.CacheEntry, *interfaces.CacheMetrics, error) {
	// Validate path to prevent path traversal
	if err := fs.validatePath(path); err != nil {
...
func (fs *FilesystemStorage) Validate() error {
	// Check if cache directory exists
	if _, err := os.Stat(fs.cacheDir); os.IsNotExist(err) {
		return fmt.Errorf("cache directory does not exist: %s", fs.cacheDir)
	}
...
func (fs *FilesystemStorage) GetLocation() string {
	return fs.cacheDir
}

// SetConfig updates the storage configuration.
func (fs *FilesystemStorage) SetConfig(config *interfaces.CacheConfig) {
	fs.config = config
}

// validatePath validates that a path is safe to use (prevents path traversal)
func (fs *FilesystemStorage) validatePath(path string) error {
