...
func (ui *InteractiveUI) displayMenu(config interfaces.MenuConfig, currentIndex int) {
	ui.clearScreen()
	...
	for i, option := range config.Options {
		...
		shortcut := ""
		if option.Shortcut != "" {
			shortcut = ui.colorize(fmt.Sprintf(" [%s]", option.Shortcut), "gray")
		}

		fmt.Printf("%s%s%s%s\n", prefix, icon, label, shortcut)

		if option.Description != "" && i == currentIndex {
...
// handleMenuInput processes user input for menu navigation
func (ui *InteractiveUI) handleMenuInput(input string, config *interfaces.MenuConfig, currentIndex *int) (*interfaces.MenuResult, bool) {
	...
	case "h", "help":
		if config.ShowHelp {
			ui.showContextHelp(config.HelpText, "menu")
			return nil, false
		}
	...
	case "", "enter":
		if *currentIndex >= 0 && *currentIndex < len(config.Options) {
			option := config.Options[*currentIndex]
			if !option.Disabled {
	...
	default:
		...
		for i, option := range config.Options {
			if option.Shortcut != "" && strings.ToLower(option.Shortcut) == input {
				if !option.Disabled {
					return &interfaces.MenuResult{
						SelectedIndex: i,
						SelectedValue: option.Value,
...
func (ui *InteractiveUI) displayMultiSelect(config interfaces.MultiSelectConfig, currentIndex int, searchQuery string, filteredIndices []int) {
	ui.clearScreen()
	ui.showHeader(config.Title, config.Description)

	if config.SearchEnabled && searchQuery != "" {
		fmt.Printf("Search: %s\n\n", ui.colorize(searchQuery, "cyan"))
	}
...
// handleMultiSelectInput processes user input for multi-selection
func (ui *InteractiveUI) handleMultiSelectInput(input string, config *interfaces.MultiSelectConfig, currentIndex *int, searchQuery *string, filteredIndices *[]int) (*interfaces.MultiSelectResult, bool) {
	...
	case "h", "help":
		if config.ShowHelp {
			ui.showContextHelp(config.HelpText, "multiselect")
			return nil, false
		}
	...
	case "", "enter", "done":
		...
		if config.MinSelection > 0 && len(selectedIndices) < config.MinSelection {
			ui.showError(fmt.Sprintf("Please select at least %d options", config.MinSelection))
			return nil, false
		}

		if config.MaxSelection > 0 && len(selectedIndices) > config.MaxSelection {
			ui.showError(fmt.Sprintf("Please select at most %d options", config.MaxSelection))
			return nil, false
		}
	...
	case "up", "k":
		if *currentIndex > 0 {
			...
			*currentIndex = len(*filteredIndices) - 1
		}

	case "down", "j":
		if *currentIndex < len(*filteredIndices)-1 {
			*currentIndex++
		} else {
			*currentIndex = 0
		}
	...
	case " ", "space":
		if *currentIndex >= 0 && *currentIndex < len(*filteredIndices) {
			idx := (*filteredIndices)[*currentIndex]
	...
	case "/":
		if config.SearchEnabled {
			*searchQuery = ui.promptSearch()
			*filteredIndices = ui.filterOptions(config.Options, *searchQuery)
			*currentIndex = 0
		}
	...
	default:
		...
		if config.SearchEnabled && len(input) > 0 && !strings.HasPrefix(lowerInput, "/") {
			*searchQuery = input
			*filteredIndices = ui.filterOptions(config.Options, *searchQuery)
			*currentIndex = 0
		}
...
// showMultiSelectHelp displays navigation help for multi-select
func (ui *InteractiveUI) showMultiSelectHelp(allowBack, allowQuit, showHelp, searchEnabled bool) {
	if !ui.config.ShowShortcuts {
		return
	}
...
func (ui *InteractiveUI) promptSearch() string {
	fmt.Print("Search: ")
	input, _ := ui.readInput()
