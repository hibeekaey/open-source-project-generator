...
version: 1.0.0
description: Complex multi-component template
files:
  ...
  - source: backend/cmd/main.go.tmpl
    conditions:
      - name: has_backend
        ...
        value: true
        operator: eq
  - source: mobile/android/MainActivity.kt.tmpl
    conditions:
      - name: has_android
        component: mobile
        value: true
        operator: eq
  - source: infrastructure/terraform/main.tf.tmpl
    conditions:
      - name: has_terraform
...
Generated by {{.Organization}}`
	...
	// Create templates with various features
...
export default App;`,
  ...
  "dependencies": {
    "next": "{{if .Versions.Packages.next}}{{.Versions.Packages.next}}{{else}}14.0.0{{end}}",
    "react": "{{if .Versions.Packages.react}}{{.Versions.Packages.react}}{{else}}18.2.0{{end}}"
  }
}`,
...
func main() {
	...
	log.Println("Server running on port 8080")
		...
		"mobile/android/MainActivity.kt.tmpl": `package {{snakeCase .Organization}}.{{snakeCase .Name}}
...
class MainActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
		...
		"infrastructure/terraform/main.tf.tmpl": `# {{.Name}} Infrastructure
...
resource "aws_instance" "{{snakeCase .Name}}" {
  ...
  tags = {
    ...
    Organization = "{{.Organization}}"
		...
		"frontend/public/assets/logo.svg": `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100">
  <circle cx="50" cy="50" r="40" fill="#007acc"/>
  <text x="50" y="55" text-anchor="middle" fill="white">F</text>
</svg>`,
...
{{end}}`,
	...
	expectedFiles := map[string][]string{
		...
		"backend/cmd/main.go": {
			"package main",
			"Starting FullStackApp server...",
		},
		"mobile/android/MainActivity.kt": {
			"package tech_corp.full_stack_app",
			"class MainActivity : ComponentActivity()",
			"// FullStackApp Android App",
		},
	...
	// Verify binary assets are copied correctly
	logoPath := filepath.Join(outputDir, "frontend/public/assets/logo.svg")
	if _, err := os.Stat(logoPath); os.IsNotExist(err) {
		t.Error("Logo asset should be copied")
	}
